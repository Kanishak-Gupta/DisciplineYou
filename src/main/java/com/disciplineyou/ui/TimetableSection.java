...
public class TimetableSection extends VBox {
    ...
    private void buildUI() {
        ...
        actions.getChildren().addAll(editTimetableBtn, overrideBtn);
    ...
    private HBox createClassRow(TimetableEntry entry, TimetableOverride override) {
        ...
        if (isCancelled) {
            ...
            row.getChildren().add(cancelBadge);
        } else {
            // Attendance buttons
    ...
    private void showEditTimetableDialog() {
        ...
        for (TimetableEntry entry : existing) {
            ...
            Label entryLabel = new Label(entry.getDayName() + " " + entry.getStartTime() + "-" + entry.getEndTime()
        ...
        addGrid.add(new Label("Start:"), 2, 0);
        addGrid.add(startField, 3, 0);
        addGrid.add(new Label("End:"), 4, 0);
        ...
        addBtn.setOnAction(e -> {
            ...
            HBox entryRow = new HBox(8);
            entryRow.setAlignment(Pos.CENTER_LEFT);
            Label entryLabel = new Label(dayCombo.getValue() + " " + startField.getText() + "-" + endField.getText()
    ...
    private void showOverrideDialog() {
        ...
        Runnable loadClasses = () -> {
            ...
            for (TimetableEntry entry : entries) {
                ...
                Label lbl = new Label(entry.getStartTime() + "-" + entry.getEndTime() + " " + entry.getSubjectName());
